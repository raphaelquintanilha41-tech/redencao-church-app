  ...
  return (
    <div className="static-screen">
      <header className="igreja-header">
        ...
        <h1 className="igreja-title">Testemunhos</h1>
      </header>
      {!showForm && (
      ...
      {showForm && (
        <section className="card perfil-section">
          <h3 className="home-section-title">Seu testemunho</h3>
          <textarea
          ...
          />
          <p className="home-event-meta">Seu testemunho passa por aprovação antes de aparecer publicamente.</p>
      ...
      {loading ? (
        ...
        <div className="caminhada-empty-state">
          <p className="static-body-text">Ainda não há testemunhos aprovados publicados. Seja o primeiro a compartilhar o seu.</p>
        </div>
      ) : (
        testimonies.map((t) => (
          <section key={t.id} className="card perfil-section">
            <p className="static-body-text">{t.content}</p>
          </section>
        ))
      )}
