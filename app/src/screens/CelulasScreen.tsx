  ...
  return (
    <div className="static-screen">
      <header className="igreja-header">
        ...
        <h1 className="igreja-title">Células</h1>
      </header>
      ...
      {loading ? (
        ...
        filtered.map((g) => (
          <section key={g.id} className="card caminhada-cell-card">
            <span className="agenda-card-category">{g.category}</span>
            <div className="home-plan-title">{g.name}</div>
            <div className="home-event-meta">Líder: {g.leader}</div>
            <div className="home-event-meta">{g.day_time}</div>
            <div className="home-event-meta">{g.region}</div>
            {g.description && <p className="static-body-text">{g.description}</p>}
            <button
              type="button"
